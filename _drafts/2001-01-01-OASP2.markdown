...
傅里叶变换是非常有用的工具；不仅在时序上可以确定周期/频率，之后还会看到一个光谱仪也相当于一个傅里叶变换器。
...
这里给出了常见的傅里叶变换以及图像。fft的结果因为是取模后画的图所以相当于解析的结果取了绝对值，同时因为离散化非常接近
...
可以看到最终傅里叶域的函数被乘上了一个$$ i(\sigma) $$。一般仪器轮廓是一个比较窄的高斯或者洛仑兹轮廓，所以它的傅里叶变换是一个相对宽的轮廓，对傅里叶域中的“低频”部分没有很大影响；但是轮廓会在“高频”部分下降，所以会抹掉“高频”部分的信息。在频率域上这意味着每个数据点不再是一个孤立的点，而因为与仪器轮廓的卷积会和旁边的数据点耦合，将高频的信息“平均”掉。

### 采样 $$ III(\sigma) $$的影响

采样对高频信号的也会产生影响（可能比仪器轮廓更大）。信号与Shah函数的卷积$$ III(\sigma) * f(\sigma) $$会使得信号在Shah函数每个不为0处附近重复，使得一样的图样发生重叠，同样抹掉了高频的信息。因为Shah函数傅里叶变换后成为间隔为$$ \frac{1}{\Delta x} $$的Shah函数，所以我们认为在这个间隔的一半区间内，信号是没有重叠的，叫做内奎斯特频率：

$$ \sigma_N = \frac{1}{2\Delta x} $$

在这里因为我们对信号进行了采样，所以得到的实际上是离散的信号点；我们并不知道点和点之间究竟是怎么样的（虽然经常认为是平滑过渡的），而点和点之间丢失的信息就对应着频域中大于内奎斯特频率的部分，它们重叠了。与仪器轮廓的不同在于采样是离散化，而仪器轮廓是模糊。

我们想做的是通过测量$$ D(x) $$求得$$ d(\sigma) $$，再转换成$$ f(\sigma) $$最后求得$$ F(x) $$。从$$ d(\sigma) $$到$$ f(\sigma) $$这一步有$$ III(\sigma) $$的作用，它们都抹掉了高频的信息。如果$$ f(\sigma) $$在高频处趋于0，那么转换这一步基本上是正确的。但是如果重叠的部分很宽，这部分就不会有唯一的解，解决办法是减少采样的间隔。实际上$$ f(\sigma) $$一般的确在高频处趋于0，而且$$ i(\sigma) $$的存在也会强制使得$$ f(\sigma) $$高频的值减小，所以转换是成立的。
...
FFT中的输入和结果都是离散的，也就是说$$ D(x) $$变成了$$ D(j\Delta x) $$，$$ j $$是整数。同理，将结果也离散化成$$ \sigma = k \Delta \sigma $$，从傅里叶变换的定义出发我们可以得到

$$ d(\sigma) = d(k) = \sum_1^N D(j)e^{2\pi ijk\Delta x \Delta\sigma} \Delta x $$
...
$$  d(k) = \sum_1^N D(j)e^{2\pi ijk/N} \Delta x $$

其实现在就是一个FFT命令的事情。

##### 有关numpy中的fft

因为采样的缘故，fft只计算到内奎斯特频率的位置。而如果原函数不是实数函数，那么傅里叶变换关于0点对称的复数值将不会共轭，所以numpy中的fft先计算0到$$ \sigma_N $$的值，再计算0到$$ -\sigma_N $$的值，并使傅里叶变换的长度与源函数长度一致（一半分给正频率，一半给负频率）。这给我们画图带来了一些麻烦，所以numpy提供了`numpy.fft.fftfreq(n[, d])`来给出傅里叶变换的x值，只需输入原函数长度和步长即可。上面的画图中因为结果已知而且基本上特征都在0附近，所以人为地截断到了和原函数同样的x值上；也能看出fft的结果分辨率比解析的差很多，是截断带来的后果。
...
当误差与$$ x,\sigma $$无关，并且采样间隔小到除了$$ \sigma $$域的主体之外，还有一部分高频的变换也在$$ \sigma_N $$之内时，我们通过可以高频部分直接测量$$ S_\sigma $$从而算出$$ S_x $$。
